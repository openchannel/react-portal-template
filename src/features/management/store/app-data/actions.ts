...
export const setReducer = (type:string, val: { data: FullAppData; periodVal: string; fieldLabel: string; appVal: string; }) => {
...
export const appVersions = () => async (dispatch: Dispatch) => {
    ...
    try {
      const { data } = await AppVersionService.getAppsVersions(appsConfig.data.pageNumber, appsConfig.data.count, sortQuery, JSON.stringify(query))
...
export const updateChartData = (period:ChartStatisticFiledModel, field:ChartStatisticFiledModel, app:ChartStatisticFiledModel) => async (dispatch: Dispatch) => {
    const dateEnd = new Date();
    const dateStart = chartService.getDateStartByCurrentPeriod(dateEnd, period!);
	  const appReq = (app!.id !== 'allApps' ? app!.id : '');

    try {
		  const { data } = await chartService.getTimeSeries(period!.id, field!.id, dateStart.getTime(), dateEnd.getTime(), appReq);
      dispatch(setReducer( ActionTypes.SET_APP, {data, periodVal: period!.id, fieldLabel: field!.label, appVal: app!.id }));
